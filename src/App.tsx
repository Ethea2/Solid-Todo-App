...
import { CommandPalette } from './keys'
...
const App: Component = () => {
  ...
  let inputBox
  fetchTodoData().then(setTodoList) //ask lyon peeps
  const addTodo = (label) => {
    if (label.trim() === "") {
  ...
  const deleteData = (label) => {
    ...
    //inItem(todoList(), label) //make removing items reactive
    setTodoList((currentList) => currentList.filter((todoObject) => !Object.values(todoObject).includes(label)))
  }
  let input: string
  return (
    <>
